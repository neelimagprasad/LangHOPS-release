...
from .models._CONSTANTS import LVIS_CATEGORIES, OBJ365_CATEGORIESV2, OPENIMAGE_CATEGORIES, OPENIMAGES_PART_CATEGORIES, OPENIMAGES_OBJECT_CATEGORIES, \
                              ...
                              PASCAL_JOINT_CATEGORIES, PASCAL_OBJECT_CATEGORIES, PASCAL_PART_CATEGORIES, PARTIMAGENET_JOINT_CATEGORIES, PARTIMAGENET_JOINT_RENAME_CATEGORIES, PARTIMAGENET_OBJECT_CATEGORIES, PARTIMAGENET_PART_CATEGORIES, \
                              PARTIMAGENET_SEMSEG_OBJECT_CATEGORIES, PARTIMAGENET_SEMSEG_PART_CATEGORIES, \
                              COCO_PANOPTIC_CATEGORIES, PASCAL_OPEN_VOCABULARY_JOINT_BASE_CATEGORIES, PASCAL_BASE_PART_CATEGORIES, \
                              PASCAL_VAL_PART_CATEGORIES, PASCAL_VAL_OBJECT_CATEGORIES, PASCAL_OPEN_VOCABULARY_JOINT_VAL_CATEGORIES, SEGINW_CATEGORIES, PASCAL_VOC_CATEGORIES, OBJECT_LEVEL_DATASETS, PARTIMAGENET_SEMSEG_OBJECT_CATEGORIES, PARTIMAGENET_SEMSEG_PART_CATEGORIES
from .models._INFERENCE_CONSTANTS import DATASET_SPECIFIC_CATEGORIES, PART_DATASETS_OBJECT_CATEGORY_INDEX, PART_DATASETS_PART_CATEGORY_INDEX, DATASET_CATEGORY_SET, DATASET_PART_NUMS
...
def check_grad_strides(name):
    def hook(grad):
        if grad.shape == (2048, 256, 1, 1):
            print(f"[MATCHING WARNING] Grad hook - {name}")
            print(f"  Shape: {grad.shape}")
            print(f"  Stride: {grad.stride()}")
            print(f"  Contiguous: {grad.is_contiguous()}")
    return hook
...
class PartGLEE(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.object_detach = cfg.MODEL.MaskDINO.OBJECT_DETACH
        self.unify_object_part = cfg.MODEL.MaskDINO.UNIFY_OBJECT_PART
        self.partglee = PartGLEE_Model(cfg, matcher, self.device, self.video_info, cfg.MODEL.CONTRAS_MEAN, self.unify_object_part, data_classes_dict = self.dataset_name_dicts, detach_object_queries = self.object_detach)

        # for name, param in self.partglee.named_parameters():
        #     if param.requires_grad:
        #         param.register_hook(check_grad_strides(name))
        ...
        self.visualize_and_save = cfg.MODEL.MaskDINO.TEST.VISUALIZE_AND_SAVE
    ...
    def forward(self, batched_inputs):
        ...
        is_sem_seg = False
        if 'is_sem_seg' in batched_inputs[0]:
            is_sem_seg = batched_inputs[0]['is_sem_seg']
        

        if self.training:
            images = self.preprocess_image(batched_inputs, task)
            images_vlm = self.preprocess_image_vlm(batched_inputs)
            ...
            if task in ['vis', 'ovis', 'ytvis19' ,'ytvis21', 'lvvis', 'uvo_video','burst', 'tao_video']:
                ...
                images_vlm = self.preprocess_image_vlm(batched_inputs)
                outputs = self.partglee(images, prompt_list, task, batch_name_list=batch_name_list, is_train=False, custom_object_categories_idx=custom_object_categories_idx, custom_part_categories_idx=custom_part_categories_idx, images_vlm = images_vlm)
                ...
                # print("mask_cls_result[0]: ", mask_cls_results[0].shape)
                # print("mask_pred_result[0]: ", mask_pred_results[0].shape)
                bid = 0
                for mask_cls_result, mask_pred_result, mask_box_result, input_per_image, image_size in zip(
                    ...
                    if self.semantic_on:
                        ...
                        if task == 'ade20k' or task == 'pascalvoc' or task == 'partimagenet_semseg':
                            ...
                            if task == 'ade20k':
                                ...
                                ignore_label = 255
                            elif task == 'partimagenet_semseg':
                                ignore_label=65535
                                test_obj_classes = PARTIMAGENET_SEMSEG_OBJECT_CATEGORIES
                                test_classes = PARTIMAGENET_SEMSEG_PART_CATEGORIES


                            for i in torch.unique(batched_inputs[0]["sem_seg"]):
                                if i != ignore_label:
                                    if i not in list(range(len(test_obj_classes))):
                                        print("task: ", task)
                                        print("len(test_obj_classes): ", test_obj_classes)
                                        print("torch.unique(batched_inputs[0]['sem_seg']): ", torch.unique(batched_inputs[0]["sem_seg"]))
                                        print("batched_inputs[0]['file_name']: ", batched_inputs[0]["file_name"])
                                        print("batched_inputs[0].keys(): ", batched_inputs[0].keys())
                                        exit(0)
                    ...
                    if self.instance_on:
                        ...
                        if self.visualize_and_save:
                            image = batched_inputs[bid]['image']
                            # print(f"Original image shape: {image.shape}")
                            image = image.permute(1, 2, 0).cpu().numpy()
                            # new_height, new_width = instance_r.pred_masks.shape[1], instance_r.pred_masks.shape[2]
                            # print(f"Original image shape: {image.shape}")
                            # print(f"Target size: {new_width}x{new_height}")
                            # resized_image = cv2.resize(image, (new_width, new_height), interpolation=cv2.INTER_LINEAR)
                            dataset_name = batched_inputs[bid]['dataset_name']
                            annotations = batched_inputs[bid]['instances']
                            metadata = MetadataCatalog.get(dataset_name + "_val")
                            visualizer = Visualizer(resized_image, metadata, instance_mode=True)
                            filename = batched_inputs[bid]['file_name'].split('/')[-1]
                            
                            gt_dir = os.path.join(self.cfg.OUTPUT_DIR, 'vis', dataset_name, 'gt')
                            out_dir = os.path.join(self.cfg.OUTPUT_DIR, 'vis', dataset_name, 'out')
                            ...
                            # print("instance_r.pred_masks.shape: ", instance_r.pred_masks.shape)
                            # print("self.cfg.INPUT.IMAGE_SIZE: ", self.cfg.INPUT.IMAGE_SIZE)
                                
                            vis_gt = visualizer.draw_instance_predictions(predictions=annotations.to(self.cpu_device))
                            vis_gt.save(os.path.join(gt_dir, filename))
                            vis_output = visualizer.draw_instance_predictions(predictions=instance_r.to(self.cpu_device))
                            vis_output.save(os.path.join(out_dir, filename))
    ...
    def prepare_targets_object_part(self, batched_inputs, targets, images, task, is_sem_seg):
        ...
        if not is_sem_seg: 
            ...
            assert task in ['ade20k','pascalvoc','partimagenet_semseg']
            # print("files: ", [x['sem_seg_file_name'] for x in batched_inputs])
            for targets_per_image in targets:
                obj_sem_targets, part_sem_targets = targets_per_image
                if isinstance(obj_sem_targets.gt_masks, torch.Tensor):
                    gt_masks_obj = obj_sem_targets.gt_masks
                    gt_masks_part = part_sem_targets.gt_masks
                else:
                    gt_masks_obj = obj_sem_targets.gt_masks.tensor
                    gt_masks_part = part_sem_targets.gt_masks.tensor
                    
                padded_masks_obj = torch.zeros((gt_masks_obj.shape[0], h_pad, w_pad), dtype=gt_masks_obj.dtype, device=gt_masks_obj.device)
                padded_masks_obj[:, : gt_masks_obj.shape[1], : gt_masks_obj.shape[2]] = gt_masks_obj
                padded_masks_part = torch.zeros((gt_masks_part.shape[0], h_pad, w_pad), dtype=gt_masks_part.dtype, device=gt_masks_part.device)
                padded_masks_part[:, : gt_masks_part.shape[1], : gt_masks_part.shape[2]] = gt_masks_part
                
                object_level_task = task + '_object'
                part_level_task = task + '_part'
                object_category_indices = torch.tensor(self.object_category_index_mapper[object_level_task]).to(self.device)
                part_category_indices = torch.tensor(self.part_category_index_mapper[part_level_task]).to(self.device)
                part_category_indices = part_category_indices - len(self.object_category_index_mapper[object_level_task])
                
                image_size_xyxy = torch.as_tensor([w_pad, h_pad, w_pad, h_pad], dtype=torch.float, device=self.device)
                gt_classes_obj = obj_sem_targets.gt_classes
                gt_boxes_obj = box_ops.box_xyxy_to_cxcywh(obj_sem_targets.gt_boxes.tensor)/image_size_xyxy
                gt_boxes_obj = torch.clamp(gt_boxes_obj,0,1)
                gt_classes_part = part_sem_targets.gt_classes
                gt_boxes_part = box_ops.box_xyxy_to_cxcywh(part_sem_targets.gt_boxes.tensor)/image_size_xyxy
                gt_boxes_part = torch.clamp(gt_boxes_part,0,1)
                # print("gt_classes_part: ", gt_classes_part)
                # print("gt_classes_obj: ", gt_classes_obj)
                object_category_mask = torch.isin(gt_classes_obj, object_category_indices)
                # part_category_mask = ~object_category_mask
                part_category_mask = torch.isin(gt_classes_part, part_category_indices)
                
                # print("task: ", task)
                # print("part_category_indices: ", part_category_indices)
                # print("gt_classes_obj: ", gt_classes_obj[object_category_mask])
                # print("gt_classes_part: ", gt_classes_part[part_category_mask])
                
                object_targets.append(
                    {
                        "labels": gt_classes_obj[object_category_mask],
                        "masks": padded_masks_obj[object_category_mask],
                        "boxes": gt_boxes_obj[object_category_mask],
                    })
                part_targets.append(
                    {
                        "labels": gt_classes_part[part_category_mask],
                        "masks": padded_masks_part[part_category_mask],
                        "boxes": gt_boxes_part[part_category_mask],
                    })
    ...
    def preprocess_image_vlm(self, batched_inputs):
        if self.use_llm:
            images_vlm = []
            for i, batch_item in enumerate(batched_inputs):
                image = batch_item["image"]
                image_max = image.max()
                image_min = image.min()
                image = (image - image_min) / (image_max - image_min + 1e-8)  # avoid div by zero
                images_vlm.append(image)
            return images_vlm
        else:
            return None 
