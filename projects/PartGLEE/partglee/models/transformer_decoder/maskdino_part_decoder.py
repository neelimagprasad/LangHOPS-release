...
class MaskDINOPartDecoder(nn.Module):
    ...
    def from_config(cls, cfg, in_channels, lang_encoder, mask_classification, is_part_decoder):
        ...
        ret["use_llm"] = cfg.MODEL.LLM.USE_LLM
        return ret

    def prepare_for_dn(self, targets, tgt, refpoint_emb, batch_size, task, topk, num_part_queries, part_query_attn_mask):
        """
        ...
        if self.training:
            ...
            if self.unify_object_part and (self.use_qformer or self.use_llm):
                # tgt_size = pad_size + topk*num_part_queries # assumption is made under fixed length of learnalbe queries
                tgt_size = pad_size + num_part_queries
            ...
            if part_query_attn_mask is None:  # atten mask across the batch is the same
                attn_mask = torch.zeros((tgt_size, tgt_size), dtype = torch.bool).to('cuda')
            ...
            else: # different batch size need different attn mask
                attn_mask = torch.zeros((tgt_size, tgt_size), dtype = torch.bool).to('cuda')
                # match query cannot see the reconstruct
                attn_mask[pad_size:, :pad_size] = True
                # reconstruct cannot see each other
                ...
                for bid in range(batch_size):
                    # mask out padded part queries
                    part_query_pad_idx = part_query_attn_mask[bid]
                    attn_mask[bid, :, (pad_size + part_query_pad_idx):] = True
                    
                # broadcase attn mask to multi heads
                attn_mask = attn_mask.unsqueeze(1).repeat(1, self.num_heads, 1, 1)
                ## to (batch_size * num_heads, tgt_size, tgt_size)
            ...
            if part_query_attn_mask is None:
                attn_mask = None
            else:
                attn_mask = torch.zeros((batch_size, num_part_queries, num_part_queries), dtype = torch.bool).to('cuda')
                for bid in range(batch_size):
                    # mask out padded part queries
                    part_query_pad_idx = part_query_attn_mask[bid]
                    attn_mask[bid, :, part_query_pad_idx:] = True
                attn_mask = attn_mask.unsqueeze(1).repeat(1, self.num_heads, 1, 1)
                ## to (batch_size * num_heads, tgt_size, tgt_size)
                attn_mask = attn_mask.flatten(0, 1)
    ...
    def forward(self, x, mask_features, extra, task, masks, targets=None, part_queries=None, topk=None, num_part_queries=None, custom_part_categories_idx=None, part_query_attn_mask=None):
        """
        ...
        predictions_mask = []
        if self.two_stage:
            if self.use_qformer or self.use_llm:
                ...
                if task in ['grounding','rvos']:
                    ...
                    if self.unify_object_part and task not in self.object_level_datasets:
                        ...
                        # print("extra['class_embeddings'].shape: ", extra['class_embeddings'].shape)
                        # print("part_category_idx: ", part_category_idx)
        ...
        if self.dn != "no" and self.training:
            ...
            input_query_label, input_query_bbox, tgt_mask, mask_dict = \
                self.prepare_for_dn(targets, None, None, x[0].shape[0], task, topk, num_part_queries, part_query_attn_mask)
    ...
    def forward_prediction_heads(self, output, mask_features, task, extra, mask_dict, pred_mask=True, visual_P=False, custom_part_categories_idx=None):
        ...
        if torch.isnan(outputs_class).any():
            print("torch.isnan(extra['class_embeddings']).any(): ", torch.isnan(extra['class_embeddings']).any())
            print("torch.isnan(class_embed).any(): ", torch.isnan(class_embed).any())
