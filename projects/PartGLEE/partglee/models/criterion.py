...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, task, object_outputs, topk_object_queries_indices, use_dn, log=True):
        ...
        if torch.isnan(loss_ce).any():
            print("loss_ce.shape: ", loss_ce.shape)
            print("src_logits.shape: ", src_logits.shape)
            print("target_classes_onehot.shape: ", target_classes_onehot.shape)
            print("src_logits NaNs:", torch.isnan(src_logits).any())
            print("src_logits Infs:", torch.isinf(src_logits).any())
            print("target_classes_onehot unique:", torch.unique(target_classes_onehot))

            # print("src_logits[torch.isnan(loss_ce)]: ", src_logits[torch.isnan(loss_ce)])
            # print("target_classes_onehot[torch.isnan(loss_ce)]: ", target_classes_onehot[torch.isnan(loss_ce)])
        losses = {
