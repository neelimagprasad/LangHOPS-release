...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets, task, cost=["cls", "box", "mask"]):
        ...
        for b in range(bs):
            ...
            look_into_bbox = box_cxcywh_to_xyxy(out_bbox)
            if not (look_into_bbox[:, 2:] >= look_into_bbox[:, :2]).all():
                print("look_into_bbox: ", look_into_bbox[ (look_into_bbox[:, 2:] < look_into_bbox[:, :2]).sum(dim=1) ])
            if 'box' in cost:
