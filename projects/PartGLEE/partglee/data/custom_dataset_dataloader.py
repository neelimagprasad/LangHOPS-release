...
def get_detection_dataset_dicts_with_source(
    ...
    for di, dataset_name in enumerate(dataset_names):
        dataset_sizes_dict[dataset_name] = len(dataset_dicts[di])
    print("dataset_sizes_dict: ", dataset_sizes_dict)
    ...
    for i in range(len(dataset_dicts)):
        has_instances = "annotations" in dataset_dicts[i][0]
        if filter_empty and has_instances:
            dataset_dicts[i] = filter_images_with_only_crowd_annotations(dataset_dicts[i])
        if min_keypoints > 0 and has_instances:
            dataset_dicts[i] = filter_images_with_few_keypoints(dataset_dicts[i], min_keypoints)
