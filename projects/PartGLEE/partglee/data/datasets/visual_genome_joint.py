...
root = os.getenv("DETECTRON2_DATASETS", None)
with open(os.path.join(root, "visual_genome/vg_object_category.txt"), 'r') as file:
    vg_object_categories = file.readlines()
    VG_OBJECT_CATEGORIES = [line.strip() for line in vg_object_categories]

with open(os.path.join(root, "visual_genome/vg_part_category.txt"), 'r') as file:
