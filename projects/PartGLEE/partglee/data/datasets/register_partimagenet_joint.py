...
PARTIMAGENET_JOINT_RENAME_CATEGORIES = [
    {"id": 0, "name": "Four-legged Animal"}, 
    {"id": 1, "name": "Monkey"},
    {"id": 2, "name": "Fish"},
    {"id": 3, "name": "Bird"},
    {"id": 4, "name": "Snake"},
    ...
    {"id": 9, "name": "Aeroplane"},
    {"id": 10, "name": "Bottle"},
    {"id": 11, "name": "Four-legged Animal Head"},
    {"id": 12, "name": "Four-legged Animal Body"},
    {"id": 13, "name": "Four-legged Animal Foot"},
    {"id": 14, "name": "Four-legged Animal Tail"},
    {"id": 15, "name": "Monkey Head"},
    {"id": 16, "name": "Monkey Body"},
    {"id": 17, "name": "Monkey Hand"},
    {"id": 18, "name": "Monkey Foot"},
    {"id": 19, "name": "Monkey Tail"},
    {"id": 20, "name": "Fish Head"},
    {"id": 21, "name": "Fish Body"},
    {"id": 22, "name": "Fish Fin"},
    {"id": 23, "name": "Fish Tail"},
...
_PARTIMAGENET_JOINT = {
    ...
    "partimagenet_renamed_joint_val": ("partimagenet/val/", "partimagenet/partimagenet_renamed_val_joint.json"),
    "partimagenet_renamed_joint_train": ("partimagenet/train/", "partimagenet/partimagenet_renamed_train_joint.json"),
}
...
def _get_partimagenet_metadata():
    ...
    obj_ids = [thing_dataset_id_to_contiguous_id[item['id']] for item in PARTIMAGENET_OBJECT_CATEGORIES]
    
    return {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes,
        'obj_ids': obj_ids, 
        'part_ids': [thing_dataset_id_to_contiguous_id[item['id']] for item in PARTIMAGENET_JOINT_CATEGORIES if thing_dataset_id_to_contiguous_id[item['id']] not in obj_ids and item['id'] in thing_dataset_id_to_contiguous_id], 
        }

def _get_partimagenet_renamed_metadata():
    id_to_name = {x['id']: x['name'] for x in PARTIMAGENET_JOINT_RENAME_CATEGORIES}
    thing_dataset_id_to_contiguous_id = {
        x: i for i, x in enumerate(sorted(id_to_name))}
    thing_classes = [id_to_name[k] for k in sorted(id_to_name)]
    obj_ids = [thing_dataset_id_to_contiguous_id[item['id']] for item in PARTIMAGENET_OBJECT_CATEGORIES]
    ...
    return {
        ...
        "thing_classes": thing_classes,
        'obj_ids': obj_ids, 
        'part_ids': [thing_dataset_id_to_contiguous_id[item['id']] for item in PARTIMAGENET_JOINT_RENAME_CATEGORIES if thing_dataset_id_to_contiguous_id[item['id']] not in obj_ids and item['id'] in thing_dataset_id_to_contiguous_id], 
        }
    ...
    for key, (image_root, json_file) in _PARTIMAGENET_JOINT.items():
        if 'rename' in key:
            register_coco_instances(
                key,
                _get_partimagenet_renamed_metadata(),
                os.path.join(root, json_file) if "://" not in json_file else json_file,
                os.path.join(root, image_root),
                dataset_name_in_dict="partimagenet_renamed_joint",
                evaluator_type='coco'
            )
        else:
            register_coco_instances(
