...
class SemanticObjPartDatasetMapper:
    ...
    def from_config(cls, cfg, is_train=True):
        ...
        for i in range(len(dataset_names)):
            meta = MetadataCatalog.get(dataset_names[i])
            if hasattr(meta, "task"):
                task = meta.task
                
                # ignore_label = meta.ignore_label
                if hasattr(meta, "ignore_label"):
                    ignore_label[task] = meta.ignore_label
                else:
                    ignore_label[task] = None
                if hasattr(meta, "obj_map"):
                    obj_map[task] = meta.obj_map
                else:
                    obj_map[task] = None
            
                if hasattr(meta, "obj_part_map"):
                    obj_part_map[task] = meta.obj_part_map
                else:
                    obj_part_map[task] = None
        ret = {
            "is_train": is_train,
            "augmentations": augs,
    ...
    def __call__(self, dataset_dict):
        ...
        """
        task = dataset_dict['dataset_name']
        ...
        if dataset_dict["sem_seg_file_name"] != 'NA':
            ...
            obj_part_sem_seg_gt = np.zeros(image_shape, dtype=np.float32) + self.ignore_label[task]
        ...
        if dataset_dict["obj_sem_seg_file_name"] != 'NA':
            ...
            obj_sem_seg_gt = np.zeros(image_shape, dtype=np.float32) + self.ignore_label[task]
        ...
        # print("dataset_dict['sem_seg_file_name']: ", dataset_dict["sem_seg_file_name"])
        # print("np.unique(ori_obj_part_sem_seg_gt): ", np.unique(obj_part_sem_seg_gt))
            
        dataset_dict['ori_obj_part_sem_seg_gt'] = obj_part_sem_seg_gt
        dataset_dict['ori_obj_sem_seg_gt'] = obj_sem_seg_gt
        
        if self.obj_part_map[task] is not None:
            mapped_obj_part_sem_seg_gt = np.zeros_like(obj_part_sem_seg_gt, dtype=np.float32) + self.ignore_label[task]
            for old_id, new_id in self.obj_part_map[task].items():
        ...
        # print("np.unique(mapped_obj_part_sem_seg_gt): ", np.unique(dataset_dict['ori_obj_part_sem_seg_gt']))
        ...
        if "category_id" in dataset_dict: ### obj binary mask mode
            obj_sem_seg_gt[obj_sem_seg_gt != dataset_dict["category_id"]] = self.ignore_label[task]
            obj_part_sem_seg_gt[obj_sem_seg_gt != dataset_dict["category_id"]] = self.ignore_label[task]
        
        if self.obj_map[task] is not None:
            mapped_obj_sem_seg_gt = np.zeros_like(obj_sem_seg_gt, dtype=np.float32) + self.ignore_label[task]
            for old_id, new_id in self.obj_map[task].items():
        ...
        if len(np.unique(obj_part_sem_seg_gt)) == 1 and np.unique(obj_part_sem_seg_gt)[0] == self.ignore_label[task]:
            return None
        ...
        if self.size_divisibility > 0:
            ...
            if obj_sem_seg_gt is not None:
                obj_sem_seg_gt = F.pad(obj_sem_seg_gt, padding_size, value=self.ignore_label[task]).contiguous()
            
            if obj_part_sem_seg_gt is not None:
                obj_part_sem_seg_gt = F.pad(obj_part_sem_seg_gt, padding_size, value=self.ignore_label[task]).contiguous()
        ...
        # print("obj_sem_seg_gt is None: ", obj_sem_seg_gt is None)
        if obj_sem_seg_gt is not None:
            ...
            obj_classes = obj_classes[obj_classes != self.ignore_label[task]]
            
            obj_part_classes = obj_part_classes[obj_part_classes != self.ignore_label[task]]
            obj_part_instances.gt_classes = torch.tensor(obj_part_classes, dtype=torch.int64)
            # print("obj_part_classes: ", obj_part_classes)
            ...
            if len(obj_masks) == 0:
                ...
                obj_instances.gt_boxes = Boxes(torch.tensor([[0., 0., 0., 0.]]))
            else:
                obj_masks = BitMasks(
            ...
            if len(obj_part_masks) == 0:
                ...
                obj_part_instances.gt_boxes =Boxes(torch.tensor([[0., 0., 0., 0.]]))
            else:
